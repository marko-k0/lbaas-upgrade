...
"""lbaas v1 to v2 migration

Revision ID: 4aff599b9845
Revises: 3345facd0452
Create Date: 2017-07-17 14:07:43.129791

"""

# revision identifiers, used by Alembic.
revision = '4aff599b9845'
...
def upgrade():
    ...
    # check if we can migrate pools table or not
    # we don't support moving forward if there exists a pool with multiple monitors
    multi_monitor_pools = list()
    sql_pools = "select * from (select pool_id, count(monitor_id) as ctr from poolmonitorassociations group by pool_id) as t where t.ctr > 1"
    r_m = connection.execute(sql_pools)
    for row in r_m:
        multi_monitor_pools.append(row.pool_id)

    if multi_monitor_pools:
        print("pools with multiple monitors exists, reduce them to 1 monitor:")
        for pool_id in multi_monitor_pools:
            print(" - pool id: {}".format(pool_id))
        sys.exit(-1)
    else:
        hm_set = set()
        hm_dict = dict()
        ...
        for row in r_hm:
            # if we have n pools who share 1 health monitor,
            # we need n-1 new uuids
            hm_id = row.id
            if hm_id in hm_set:
                new_id = str(uuid.uuid4())
                hm_dict.setdefault(hm_id, list()).append(new_id)
                hm_id = new_id
            else:
                hm_set.add(hm_id)
        ...
        sqlstr_pools = "select pools.id,pools.tenant_id,pools.name," \
                   ...
                   "healthmonitor_id " \
                   "from pools left join poolmonitorassociations on " \
                   "pools.id = poolmonitorassociations.pool_id where pools.status='ACTIVE';"

        r_pool = connection.execute(sqlstr_pools)
        for row in r_pool:
            hm_id = row.healthmonitor_id
            if hm_id in hm_dict.keys() and len(hm_dict[hm_id]):
                hm_id = hm_dict.get(hm_id).pop()
            op_status = 'ONLINE'
        ...
        sql_lb = "select vips.tenant_id,vips.name, vips.description," \
                 ...
                 "vip_subnet_id,pools.id AS pool_id," \
        ...
        r_lb.close()
        # XXX: not needed since lb id = pool id
